...
export default function Talos() {
  ...
  const fetchSections = async () => {
    const { data } = await api.get('/talos/sections');
    setSections(data);
    if (!initDone.current && data.length > 0) {
      // start all expanded for Talos — it's a task workspace
  ...
  const handleAddSection = async (e: React.FormEvent) => {
    e.preventDefault();
    await api.post('/talos/sections', newSection);
  ...
  return (
    <Page>
      <PageHeader>
        <HeaderLeft>
          <PageTitle>Talos</PageTitle>
          <PageSub>HVAC Recruiting · Daily Workspace</PageSub>
        </HeaderLeft>
        <HeaderRight>
          {totalTasks > 0 && (
            <DayScore>
              <ScoreNum>{doneTasks}/{totalTasks}</ScoreNum>
              <ScoreLabel>done today</ScoreLabel>
            </DayScore>
          )}
      ...
      {totalTasks > 0 && (
        <ProgressBanner>
          <ProgressTrack>
            <ProgressFill style={{ width: `${totalTasks ? (doneTasks / totalTasks) * 100 : 0}%` }} />
          </ProgressTrack>
          <ProgressPct>{totalTasks ? Math.round((doneTasks / totalTasks) * 100) : 0}% complete</ProgressPct>
        </ProgressBanner>
      )}

      {sections.length === 0 && (
        <EmptyState>
          <EmptyTitle>Set up your Talos workspace</EmptyTitle>
          <EmptyDesc>Add sections to organize your daily tasks — Business Development, Software, Operations, or whatever fits your workflow.</EmptyDesc>
            ...
            <SectionCard key={section.id} color={section.color}>
              <SectionHeader onClick={() => toggleCollapse(section.id)}>
                <SectionLeft>
                  ...
                  <TaskCount>{section.tasks.length} task{section.tasks.length !== 1 ? 's' : ''}{completedCount > 0 ? ` · ${completedCount} done` : ''}</TaskCount>
                </SectionLeft>
                  ...
                  <TaskList>
                    ...
                    {completedTasks.map(task => (
                      <TaskRow key={task.id} faded>
                        <TaskCheck checked onClick={() => handleToggleTask(task)} color={section.color} />
        ...
        <Modal onClick={() => setShowAddSection(false)}>
          <ModalBox onClick={e => e.stopPropagation()}>
            ...
            <Form onSubmit={handleAddSection}>
              ...
              <Input
                placeholder="e.g. Business Development, Software, Operations"
...
const IconBtn = styled.button<{ danger?: boolean }>`background: none; border: none; color: ${p => p.danger ? '#F07050' : '#6B5038'}; font-size: 0.85rem; padding: 4px 6px; &:hover { color: ${p => p.danger ? '#F07050' : '#C4A870'}; opacity: 0.8; }`;
...
const TaskRow = styled.div<{ faded?: boolean }>`
  display: flex; align-items: center; gap: 10px; padding: 7px 4px; border-radius: 6px;
